...
# Registered Node Id.
PLUGIN_NODE_ID = 0x0012F188
PLUGIN_NODE_NAME = "dcCameraInfernoExtend"
PLUGIN_NODE_AUTHOR_STRING = "Vu Xuan Hoa"
PLUGIN_NODE_VERSION_STRING = "0.3.5"
...
class HUDNode(OpenMaya.MPxSurfaceShape):
    node_id = OpenMaya.MTypeId(PLUGIN_NODE_ID)
    draw_db_classification = "drawdb/geometry/dcCameraInfernoExtend"
    draw_registrant_id = "dcCameraInfernoNodeExtendPlugin"
    ...
    def initialize():
        ...
        HUDNode.m_frames_per_second = nAttr.create(
            "framesPerSecond", "fps",
            OpenMaya.MFnNumericData.kDouble, 30.0)
        nAttr.readable = True
        nAttr.writable = True
        nAttr.storable = True
        nAttr.keyable = True
        ...
        data_object = string_data.create("Arial")
...
class HUDNodeDrawOverride(OpenMayaRender.MPxDrawOverride):
    ...
    def prepareForDraw(self, obj_path, camera_path, frame_context, old_data):
        ...
        point_prev = self.previous_data[0:3]
        point_prev = OpenMaya.MVector(point_prev)

        delta_frame = abs(self.previous_data[3] - frame) or 1.0
        self.previous_data = [point_now.x, point_now.y, point_now.z, frame]

        camera_speed = (point_now - point_prev).length()/delta_frame
        camera_speed_raw = camera_speed * scene_scale_factor
    ...
    def draw_rule_of_thirds(cls,
        ...
        def quick_2d_line(a_x, a_y, b_x, b_y):
            pos_a_x, pos_a_y = cls.film_coord_to_corners(
                a_x, a_y,
                film_lower_left,
                film_upper_right)
        ...
        quick_2d_line(-1, 0.333, 1, 0.333)
        quick_2d_line(-1, -0.333, 1, -0.333)
        quick_2d_line(0.333, 1, 0.333, -1)
        quick_2d_line(-0.333, 1, -0.333, -1)
        return
    ...
    def draw_field(cls,
        ...
        elif field_type == FIELD_TYPE_RULE_THIRDS_INDEX:
            cls.draw_rule_of_thirds(
                draw_manager,
                line_width * line_width_multiplier,
                line_style,
                line_color, line_alpha,
...
def initializePlugin(obj):
    ...
    try:
        plugin.registerNode(
            ...
            HUDNode.creator,
            HUDNode.initialize,
            OpenMaya.MPxNode.kSurfaceShape,
